...
int getListSize(int *firstListPosition, int *lastListPosition)
{
  int listSize;
  if (*firstListPosition == -1)
  {
...
{
  int listSize = getListSize(firstListPosition, lastListPosition);
...
void ordenateListByBubblesort(struct Account list[], int *firstListPosition, int *lastListPosition)
{
  int totalIterations = getListSize(firstListPosition, lastListPosition);
  struct Account auxilliarAccount;
  bool hasChangedPositions;

  for (int i = 0; i < totalIterations - 1; i++)
  {
    hasChangedPositions = false;
    for (int j = 0; j < totalIterations - 1; j++)
    {
      if (list[j].id > list[j + 1].id)
      {
        auxilliarAccount = list[j];
        list[j] = list[j + 1];
        list[j + 1] = auxilliarAccount;
...
int main()
{
  ...
  while (selectedInsertModeCode != END_PROGRAM_CODE)
  {
    ...
    case INSERT_IN_BEGIN_CODE:
      ...
      ordenateListByBubblesort(linearList, &firstListPosition, &lastListPosition);
